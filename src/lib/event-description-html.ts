...
export const EVENT_DESCRIPTION_BODY_CLASSES =
  `prose prose-sm dark:prose-invert max-w-none wrap-break-word [&_iframe]:max-w-full [&_iframe]:w-full [&_img]:h-auto [&_img]:max-w-full [&_video]:h-auto [&_video]:max-w-full ${EVENT_DESCRIPTION_SPACING_CLASSES}`;

/** TipTap editable surface (no `prose` — avoids odd caret/selection behavior). */
export const EVENT_DESCRIPTION_EDITOR_INNER_CLASSES =
  `min-h-[220px] px-3 py-2 text-sm leading-relaxed text-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring ${EVENT_DESCRIPTION_SPACING_CLASSES}`;
